  ...
  if (message.content != null) { // What are the odds of this happening?
    if (!message.author.bot) { // Prevents the bot to respond to itself... and potential abuse
      if (message.content.charAt(0) == '$') { // Using the "$" as a prefix
        ...
        // Remove the prefix
        while (message.content.charAt(0) === '$') { message.content = message.content.substr(1) }
        ...
        if (/^m /.test(message.content)) {
          ...
          if (/^create/.test(message.content)) {
            ...
            if (message.content) {
              Makra.findOne({ guild: message.guild.id, command: macrostr }).then(function (res) {
                ...
                } else { message.channel.sendMessage('This macro already exists!') }
              })
            } else { message.channel.sendMessage('Cannot create blank macro!') }
          } else
            ...
            if (message.content) {
              Makra.findOne({ guild: message.guild.id, command: macrostr }).then(function (res) {
                if (res) {
                  ...
                  } else { message.channel.sendMessage("Sorry, but you can't do that!") }
                } else { message.channel.sendMessage('Macro "' + macrostr + '" does not exist!') }
              })
            } else { message.channel.sendMessage('The macro cannot be blank! No changes have been done.') }
          } else
          ...
          if (/^find/.test(message.content)) {
            ...
            if (message.content.length > 1) {
              ...
              const searchfor = new RegExp(macrostr, 'i')
            ...
            } else { message.channel.sendMessage('Please, use at least 2 characters to search') }
          }
